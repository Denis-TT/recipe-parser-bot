...
class RecipeBot:
    ...
    def get_save_keyboard(self) -> ReplyKeyboardMarkup:
        """Клавиатура для сохранения рецепта (Reply-кнопки)"""
        keyboard = [
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Сохранение рецепта (Reply-кнопки)
        elif text == "✅ Да, сохранить":
            if user_id in self.temp_recipes:
                ...
                try:
                    ...
                    logger.info(f"✅ Рецепт сохранен: {recipe.get('title')}")
            ...
            if user_id in self.temp_recipes:
                del self.temp_recipes[user_id]
            await update.message.reply_text("👌 Рецепт не сохранен", reply_markup=self.get_main_keyboard())
    ...
    async def handle_url(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            logger.info(f"📦 Рецепт во временном хранилище: {recipe.get('title')}")
            ...
            # Отправляем Reply-кнопки для сохранения
            await update.message.reply_text(
    ...
    async def handle_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Обработчик inline-кнопок (только для навигации по категориям)"""
